/*******************************************************************************
 * Copyright (c) 2016 Fujitsu Services Ltd. Author: Nick Battle This file is
 * part of VDMJ. VDMJ is free software: you can redistribute it and/or modify it
 * under the terms of the GNU General Public License as published by the Free
 * Software Foundation, either version 3 of the License, or (at your option) any
 * later version. VDMJ is distributed in the hope that it will be useful, but
...
public class TCValueDefinition extends TCDefinition
{
	...
	public TCValueDefinition(NameScope scope, TCAnnotationList annotations, TCAccessSpecifier accessSpecifier,
			TCPattern p, TCType type, TCExpression exp)
	{
	...
	{
		return accessSpecifier.ifSet(" ") + pattern + (type == null ? "" : ":" + type) + " = " + exp;
	}
	...
	public void typeResolve(Environment env)
	{
		...
		if (annotations != null)
			annotations.tcResolve(this, env);
	}
	...
	{
		if (annotations != null)
			annotations.tcBefore(this, base, scope);
		...
		else
		{
			...
			{
				TypeChecker.warning(5500, "TCImplicitTypeInlayHint, " + expType,
						pattern.location.after(pattern.toString().length()));
			}
		...
		if (annotations != null)
			annotations.tcAfter(this, type, base, scope);
	}

	/**
	 * For named types, if the type is not defined within the module, we have to
	 * make it explicit, like x:M`T rather than just x:T. This is so that toStrings
	 * (with POG) can be correctly typechecked.
	 */
		...
		for (TCDefinition d : newdefs)
		{
			for (TCDefinition u : defs)
			{
				...
				if (u instanceof TCUntypedDefinition)
				{
					...
					// If this untyped definition has already been referenced
					// (TCUntypedDefinition.findName),
					// we suggest the explicit typing to allow a forward reference to get the right
					// type.
	...
	{
		return type != null ? type : (expType != null ? expType : new TCUnknownType(location));
	}
