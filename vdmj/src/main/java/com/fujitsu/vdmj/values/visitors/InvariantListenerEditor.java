/*******************************************************************************
 *
 *	Copyright (c) 2022 Nick Battle.
 *
 *	Author: Nick Battle
 *
 *	This file is part of VDMJ.
 *
 *	VDMJ is free software: you can redistribute it and/or modify
 *	it under the terms of the GNU General Public License as published by
 *	the Free Software Foundation, either version 3 of the License, or
 *	(at your option) any later version.
 *
 *	VDMJ is distributed in the hope that it will be useful,
 *	but WITHOUT ANY WARRANTY; without even the implied warranty of
 *	MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 *	GNU General Public License for more details.
 *
 *	You should have received a copy of the GNU General Public License
 *	along with VDMJ.  If not, see &lt;http://www.gnu.org/licenses/&gt;.
 *	SPDX-License-Identifier: GPL-3.0-or-later
 *
 ******************************************************************************/

package com.fujitsu.vdmj.values.visitors;

import java.util.Iterator;

import com.fujitsu.vdmj.values.InvariantValueListener;
import com.fujitsu.vdmj.values.UpdatableValue;
import com.fujitsu.vdmj.values.Value;
import com.fujitsu.vdmj.values.ValueListener;
import com.fujitsu.vdmj.values.ValueListenerList;

public class InvariantListenerEditor extends LeafValueVisitor<ValueListener, ValueListenerList, Value>
{
	@Override
	public ValueListenerList caseValue(Value node, Value root)
	{
		return newCollection();
	}
	
	@Override
	public ValueListenerList caseUpdatableValue(UpdatableValue node, Value root)
	{
		ValueListenerList all = super.caseUpdatableValue(node, root);
		
		if (node.listeners != null)
		{
			Iterator<ValueListener> iter = node.listeners.iterator();
			
			while (iter.hasNext())
			{
				ValueListener vl = iter.next();
				
				if (vl instanceof InvariantValueListener)
				{
					InvariantValueListener ivl = (InvariantValueListener)vl;
					
					if (ivl.getValue().getValue() == root)	// NB object equality
					{
						iter.remove();
						all.add(ivl);
					}
				}
			}
		}
		
		return all;
	}

	@Override
	protected ValueListenerList newCollection()
	{
		return new ValueListenerList();
	}
}
