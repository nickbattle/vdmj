...
MVERSION=${VDMJ_VERSION:-4.8.0-SNAPSHOT}
PVERSION=${VDMJ_PVERSION:-4.8.0-P-SNAPSHOT}
...
SMTLIB_JAR=$MAVENREPO/smtlib/${VERSION}/smtlib-${VERSION}.jar
...
CLASSPATH="$VDMJ_JAR:$COMMANDS_JAR:$STDLIB_JAR:$QUICKCHECK_JAR:$SMTLIB_JAR:$ANNOTATIONS_JAR:$ANNOTATIONS2_JAR:$VDMJ_ANNOTATIONS:$VDMJ_CLASSPATH"
MAIN="VDMJ"
...
then
	...
	eval exec rlwrap --no-warnings "$JAVA64" $VMOPTS -cp $CLASSPATH $MAIN -$DIALECT $VDMJOPTS
else
