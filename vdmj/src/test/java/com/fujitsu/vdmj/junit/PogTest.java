...
public class PogTest extends TestCase
{
	...
	private String[] expected =
	{
			...
			/* 18 */ "(forall mk_(a, b):(int) * (int), c:(int) * (int) &\n  is_(pre_prepostfi(mk_(a, b), c), bool))\n",
			...
			/* 29 */ "1 in set dom m\n",
			/* 30 */ "(let x : int = m(1) in\n  x in set dom m)\n",
			/* 31 */ "(let local: int -> int local(x) == m(x) in\n  (forall x:int &\n    x in set dom m))\n",
			/* 32 */ "1 in set dom m\n",
			/* 33 */ "(let x = m(1) in\n  2 in set dom m)\n",
			/* 34 */ "(let x = m(1), y = m(2) in\n  x in set dom m)\n",
			/* 35 */ "(let x = m(1), y = m(2) in\n  y in set dom m)\n",
			/* 36 */ "exists1 x in set {1, 2, 3} & (x < 10)\n",
			/* 37 */ "(forall i:int &\n  is_(pre_f1(i), bool))\n",
			/* 38 */ "(forall i:int &\n  is_(pre_f2(i), bool))\n",
			/* 39 */ "(forall n:nat &\n  n > 1 => forall arg:nat & pre_f1(arg) => pre_f1(f1(arg)))\n",
			/* 40 */ "(forall x:int -> int, n:nat &\n  n > 1 => forall arg:nat & pre_(x, arg) => pre_(x, x(arg)))\n",
			/* 41 */ "(forall n:nat &\n  n = 0 or n = 1 or rng(m) subset dom(m))\n",
			/* 42 */ "(forall x:map int to int, n:nat &\n  n = 0 or n = 1 or rng(x) subset dom(x))\n",
			/* 43 */ "forall arg:int & pre_f2(arg) => pre_f1(f2(arg))\n",
			/* 44 */ "(forall x:int -> int, y:int -> int &\n  forall arg:int & pre_(y, arg) => pre_(x, y(arg)))\n",
			/* 45 */ "(forall x:int -> int &\n  forall arg:int & pre_f2(arg) => pre_(x, f2(arg)))\n",
			/* 46 */ "(forall x:int -> int &\n  forall arg:int & pre_(x, arg) => pre_f1(x(arg)))\n",
			/* 47 */ "rng(m) subset dom(m)\n",
			/* 48 */ "(forall x:map int to int, y:map int to int &\n  rng(y) subset dom(x))\n",
			/* 49 */ "(forall x:map int to int &\n  rng(m) subset dom(x))\n",
			/* 50 */ "(forall i:int &\n  pre_f1(i))\n",
			/* 51 */ "(forall i:int &\n  i in set inds sq)\n",
			/* 52 */ "(forall x:seq of int &\n  123 in set inds x)\n",
			/* 53 */ "(forall s:set of (set of int) &\n  s <> {})\n",
			/* 54 */ "(forall s:seq of int &\n  (not (s = []) =>\n    s <> []))\n",
			/* 55 */ "(forall i:int &\n  i <> 0)\n",
			/* 56 */ "(forall i:int &\n  is_int((123 / i)))\n",
			/* 57 */ "(forall i:int &\n  i <> 0)\n",
			/* 58 */ "exists finmap1:map nat to (map int to nat1) & forall a:int, b in set {1, 2, 3} & (a < 10) => exists findex2 in set dom finmap1 & finmap1(findex2) = {a |-> b}\n",
			/* 59 */ "exists finmap1:map nat to (int) & forall a:int, b in set {1, 2, 3} & (a < 10) => exists findex2 in set dom finmap1 & finmap1(findex2) = (a + b)\n",
			/* 60 */ "(forall a:map int to int, b:map int to int &\n  forall ldom1 in set dom a, rdom2 in set dom b & ldom1 = rdom2 => a(ldom1) = b(rdom2))\n",
			/* 61 */ "(forall x:int &\n  forall m1, m2 in set {{1 |-> 2}, {2 |-> 3}, {x |-> 4}} & forall d3 in set dom m1, d4 in set dom m2 & d3 = d4 => m1(d3) = m2(d4))\n",
			/* 62 */ "forall m1 in set {{1 |-> 2}, {2 |-> 3}}, m2 in set {{1 |-> 2}, {2 |-> 3}} & forall d3 in set dom m1, d4 in set dom m2 & d3 = d4 => m1(d3) = m2(d4)\n",
			/* 63 */ "(forall n:nat &\n  is_(measure_recursive(n), nat))\n",
			/* 64 */ "(forall n:nat &\n  (not (n = 1) =>\n    (n - 1) >= 0))\n",
			/* 65 */ "(forall n:nat &\n  (not (n = 1) =>\n    measure_recursive(n) > measure_recursive(n - 1)))\n",
			/* 66 */ "dom {1 |-> false} subset inds [2, true, 7.8]\n",
			/* 67 */ "is_(([2, true, 7.8] ++ {1 |-> false}), seq of (bool | nat))\n",
			/* 68 */ "(forall t:((nat) * (nat) * (nat)) | ((nat) * (nat)) &\n  is_(t, (nat) * (nat) * (nat)))\n",
			/* 69 */ "(forall u:U &\n  exists mk_(a, b):U & mk_(a, b) = u)\n",
			/* 70 */ "(forall u:U &\n  is_(u, (nat) * (nat)))\n",
			/* 71 */ "(is_([1, 2, true], T1) and ((is_(1, bool)) and (is_(2, bool)))) or (is_([1, 2, true], T2) and inv_T2([1, 2, true]))\n",
			/* 72 */ "(forall t:T1 | T2 &\n  (is_([1, 2, 3, true], T1) and ((is_(1, bool)) and (is_(2, bool)) and (is_(3, bool)))) or (is_([1, 2, 3, true], T2) and inv_T2([1, 2, 3, true])))\n",
			/* 73 */ "(forall a:T1 | T2 | int &\n  (is_(a, T1) and (is_(a, seq of bool))) or (is_(a, T2) and inv_T2(a)))\n",
			/* 74 */ "is_({1 |-> \"2\"}, inmap nat1 to seq1 of char)\n",
			/* 75 */ "(forall n:nat1, x:nat1 &\n  (not (n < 2) =>\n    (n - 1) > 0))\n",
			/* 76 */ "(forall n:nat1, x:nat1 &\n  (not (n < 2) =>\n    (x - 1) > 0))\n",
			/* 77 */ "(forall n:nat1, x:nat1 &\n  (not (n < 2) =>\n    id(n, x) > id(n - 1, x - 1)))\n",
			/* 78 */ "(forall x:nat1, y:nat1 &\n  (x - y) >= 0)\n",
			/* 79 */ "(forall mk_(n, x):(nat1) * (nat1) &\n  (not (n < 2) =>\n    ((n - 1) > 0) and ((x - 1) > 0)))\n",
			/* 80 */ "(forall mk_(n, x):(nat1) * (nat1) &\n  (not (n < 2) =>\n    id2(mk_(n, x)) > id2(mk_((n - 1), (x - 1)))))\n",
			/* 81 */ "(forall mk_(x, y):(nat1) * (nat1) &\n  (x - y) >= 0)\n",
			/* 82 */ "(forall mk_(n, x):(nat1) * (nat1) &\n  (not (n < 2) =>\n    ((n - 1) > 0) and ((x - 1) > 0)))\n",
			/* 83 */ "(forall mk_(n, x):(nat1) * (nat1) &\n  (not (n < 2) =>\n    (let lhs = id3(mk_(n, x)), rhs = id3(mk_((n - 1), (x - 1))) in if lhs.#1 <> rhs.#1 then lhs.#1 > rhs.#1 else lhs.#2 > rhs.#2)))\n",
			/* 84 */ "(forall i:int, obj_A(iv |-> iv):A &\n  (1 = i => \n    1 in set dom m))\n",
			/* 85 */ "(forall i:int, obj_A(iv |-> iv):A &\n  (1 = i => \n    ((m(1) < 10) =>\n      1 in set dom m)))\n",
			/* 86 */ "(forall i:int, obj_A(iv |-> iv):A &\n  (1 = i => \n    (not (m(1) < 10) =>\n      2 in set dom m)))\n",
			/* 87 */ "(forall i:int, obj_A(iv |-> iv):A &\n  (not 1 = i =>\n    (2 = i => \n      3 in set dom m)))\n",
			/* 88 */ "(forall obj_A(iv |-> iv):A &\n  (let x : int = 10 in\n    -- Missing loop invariant))\n",
			/* 89 */ "(forall obj_A(iv |-> iv):A &\n  (let x : int = 10 in\n    (let iv : int = (iv + 1) in\n      (iv < 10))))\n"
		};
