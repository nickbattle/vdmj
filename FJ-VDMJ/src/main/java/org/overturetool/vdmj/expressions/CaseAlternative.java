...
public class CaseAlternative implements Serializable
{
	...
	public Type typeCheck(Environment base, NameScope scope, Type expType, Type constraint)
	{
		if (defs == null)
		{
			...
			if (pattern instanceof ExpressionPattern)
			{
				...
				Type ptype = ep.exp.typeCheck(base, null, scope, null);
		...
		Type r = result.typeCheck(local, null, scope, constraint);
		local.unusedCheck();
		return r;
	}
