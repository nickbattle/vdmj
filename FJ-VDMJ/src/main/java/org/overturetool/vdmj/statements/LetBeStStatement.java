...
public class LetBeStStatement extends Statement
{
	...
	public Type typeCheck(Environment base, NameScope scope, Type constraint)
	{
		...
		// cannot be updated. Therefore we wrap them in a local QualifiedDefinition.
		DefinitionList qualified = new DefinitionList();
		
		for (Definition d: def.getDefinitions())
		{
			qualified.add(new QualifiedDefinition(d, NameScope.LOCAL));
		}
		...
		Type r = statement.typeCheck(local, scope, constraint);
		local.unusedCheck();
		return r;
	}
