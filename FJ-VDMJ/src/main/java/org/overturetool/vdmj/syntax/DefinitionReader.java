...
public class DefinitionReader extends SyntaxReader
{
	...
	private AccessSpecifier readAccessSpecifier(boolean asyncOK, boolean pureOK)
		throws LexException, ParserException
	{
		if (dialect == Dialect.VDM_SL)
		{
			if (lastToken().is(Token.PURE))
			{
				...
				{
					nextToken();
					return new AccessSpecifier(false, false, Token.PRIVATE, true);
				}
				else
				{
		...
		return new AccessSpecifier(isStatic, isAsync, access, isPure);
	}
	...
	private DefinitionList readTypes() throws LexException, ParserException
	{
		...
		while (!newSection())
		{
			try
			{
				AccessSpecifier access = readAccessSpecifier(false, false);
	...
	public DefinitionList readOperations() throws LexException, ParserException
	{
		...
		while (!newSection())
		{
			try
			{
				AccessSpecifier access = readAccessSpecifier(dialect == Dialect.VDM_RT, true);
